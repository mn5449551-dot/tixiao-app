...
Image Agent 本身不包含系统提示词，它是一个纯粹的图片生成调用封装。提示词内容来自 Image Description Agent 的输出。

## 模型路由（resolveImageModel）

系统通过 `IMAGE_MODEL_CAPABILITIES`（定义在 `lib/constants.ts` 的 `IMAGE_MODELS`）自动路由到不同的传输方式：
...
默认模型从 `getModelSetting("model_image_generation")` 获取，回退到 `DEFAULT_IMAGE_MODEL_VALUE`。
...
3. **图片编辑**：通过 `editImage` 函数支持 mask 局部重绘（需模型支持 edits）
4. **多模型路由**：自动根据模型能力选择 chat_completions / images_generations / images/edits
5. **分辨率控制**：支持 1K、2K、4K 三种分辨率
6. **画幅比例控制**：支持自定义画幅比例
7. **错误重试**：内置 3 次重试机制，处理网络波动和服务端错误
8. **多格式解析**：支持解析多种图片响应格式（base64、data URL、URL）
...
export async function generateImageFromPrompt(
  ...
  if (resolved.transport === "images_generations") {
    // gpt-image-1 等走 /v1/images/generations
    return generateImageViaImagesGenerations({ ... });
  }

  // Gemini 等走 /v1/chat/completions
...
export async function editImage(input: {
  ...
  model?: string;
  aspectRatio?: string;
  maskDataUrl?: string;    // mask 蒙版 data URL（用于局部重绘）
}) {
  const resolved = resolveImageModel(input.model);
  if (!resolved.supportsEdits) {
    throw new Error(`图像编辑不支持当前模型：${resolved.model}`);
  }
...
### IMAGE_MODELS 常量（lib/constants.ts）
...
type ImageModelConfig = {
  value: string;           // 模型 ID
  label: string;           // 显示名称
  transport: "chat_completions" | "images_generations";  // 传输方式
  supportsReference: boolean;   // 是否支持参考图
  supportsEdits: boolean;       // 是否支持 edits API
  aspectRatios: string[];       // 支持的画幅比例
  defaultSize: Record<string, string>;  // 默认尺寸映射
};
```
...
```
POST ${DEFAULT_BASE_URL}/v1/images/edits
```
...
| `图片编辑接口返回异常: ${status}` | edits API 返回错误 |
| `图像编辑不支持当前模型` | 模型不支持 edits |
| `下载图片失败` | 从 URL 下载图片失败 |
