...
- 文案必须口语化、顺口、读出来不拗口，尽量像真实人会说的话，而不是说明书或汇报材料。
- 多用短句、动词和有节奏的表达，避免长难句、技术黑话和书面腔。
${isAppend ? "- 当前是追加生成，只新增 1 条文案，不能只是机械改写已有文案。" : ""}
- 若 imageForm=single：输出 titleMain、titleSub；titleMain 负责钩子或核心承诺，长度 6~22 字；titleSub 补足解法或结果，长度 7~31 字。
- 若 imageForm=double：输出 titleMain、titleSub、copyType；两句必须形成真正的双图关系；copyType 就是图间关系；copyType 只能从：并列、因果、递进、互补 中选择；titleMain 和 titleSub 都必须控制在 4~10 字内。
- 若 imageForm=triple：输出 titleMain、titleSub、titleExtra、copyType；三句必须形成递进、拆解或对照关系；copyType 就是图间关系；copyType 只能从：并列、因果、递进、互补 中选择；三句都必须控制在 4~10 字内。
