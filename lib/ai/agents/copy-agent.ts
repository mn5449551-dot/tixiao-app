...
格式要求：
- 若 imageForm=single：
  - 输出 titleMain、titleSub
  - titleMain 负责钩子或核心承诺
  - titleSub 补足解法或结果
- 若 imageForm=double：
  - 输出 titleMain、titleSub、copyType
  - 两句必须形成真正的双图关系，不能只是拆一句长句
  - copyType 就是图间关系
  - copyType 只能从：并列、因果、递进、互补 中选择
- 若 imageForm=triple：
  - 输出 titleMain、titleSub、titleExtra、copyType
  - 三句必须形成递进、拆解或对照关系，适合三张图分开展示
  - copyType 就是图间关系
  - copyType 只能从：并列、因果、递进、互补 中选择
