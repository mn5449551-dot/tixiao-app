...
export default function HomePage() {
  ...
  return (
    <main id="main-content" className="relative mx-auto flex min-h-screen w-full max-w-[1520px] flex-col gap-10 px-8 py-10">
      ...
      <section className="animate-fade-in-up flex items-center justify-between rounded-[28px] border border-[var(--line-soft)] bg-white/82 px-8 py-6 shadow-[var(--shadow-panel)] backdrop-blur-sm">
        ...
        {folders.length > 0 ? (
          <>
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold text-[var(--ink-950)]">我的文件夹</h2>
              <span className="text-sm text-[var(--ink-500)]">{folders.length} 个文件夹</span>
            </div>
            <FolderList folders={folders} />
          </>
        ) : (
          <div className="flex min-h-[320px] flex-col items-center justify-center rounded-[28px] border border-[var(--line-soft)] bg-white/90 p-10 text-center shadow-[var(--shadow-card)]">
            <span className="text-5xl">📁</span>
            <h3 className="mt-4 text-xl font-semibold text-[var(--ink-900)]">还没有文件夹</h3>
            <p className="mt-2 max-w-sm text-sm leading-7 text-[var(--ink-600)]">
